<div class="bg-white dark:bg-gray-800 shadow-xl sm:rounded-lg p-6">
    <div class="flex justify-between items-center mb-6">
        <div class="w-1/3">
            <x-input wire:model.live.debounce.300ms="search" type="text" class="block w-full" placeholder="Cari No. SPBJ..."/>
        </div>
        <a href="{{ route('spbjs.create') }}">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @forelse ($spbjs as $spbj)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">{{ $spbj->no_spbj }}</td>
                        <td class="px-6 py-4">{{ \Carbon\Carbon::parse($spbj->tanggal)->isoFormat('DD MMMM YYYY') }}</td>
                        <td class="px-6 py-4">{{ $spbj->company_unit_name }}</td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end space-x-4">
                                <a href="{{ route('spbjs.show', $spbj) }}" class="text-gray-400 hover:text-blue-600" title="Lihat Detail">
                                ...
                                <a href="{{ route('spbjs.edit', $spbj) }}" class="text-gray-400 hover:text-indigo-600" title="Edit">
                                ...
                                <button wire:click="delete({{ $spbj->id }})" class="text-gray-400 hover:text-red-600" title="Hapus">
                ...
                @empty
                    <tr><td colspan="4" class="px-6 py-4 text-center">Tidak ada data SPBJ.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    <div class="mt-4">{{ $spbjs->links() }}</div>
</div>