<div class="bg-white dark:bg-gray-800 shadow-xl sm:rounded-lg p-6">
    <div class="flex justify-between items-center mb-6">
        <div class="w-1/3">
            <x-input wire:model.live.debounce.300ms="search" type="text" class="block w-full" placeholder="Cari No. Survei atau Nama..."/>
        </div>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    <th class="px-6 py-3">No. Survei</th>
                    <th class="px-6 py-3">Pemohon</th>
                    <th class="px-6 py-3">Tanggal Survei</th>
            ...
            <tbody>
                @forelse ($surveys as $survey)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">{{ $survey->no_survey }}</td>
                        <td class="px-6 py-4">{{ $survey->permohonan->nama_pemohon ?? 'N/A' }}</td>
                        <td class="px-6 py-4">{{ \Carbon\Carbon::parse($survey->tanggal_survey)->isoFormat('DD MMMM YYYY') }}</td>
                        <td class="px-6 py-4">{{ $survey->unit_role }}</td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end space-x-4">
                                <a href="{{ route('surveys.show', $survey) }}" class="text-gray-400 hover:text-blue-600" title="Lihat Detail">
                                ...
                                <a href="{{ route('surveys.edit', $survey) }}" class="text-gray-400 hover:text-indigo-600" title="Edit">
                                ...
                                <button wire:click="delete({{ $survey->id }})" class="text-gray-400 hover:text-red-600" title="Hapus">
                ...
                @empty
                    <tr><td colspan="5" class="px-6 py-4 text-center">Tidak ada data survei untuk ditampilkan.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
